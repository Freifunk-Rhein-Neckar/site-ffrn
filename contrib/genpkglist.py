...
{% for device, include in includes.items() %}
    GLUON_{{ device }}_SITE_PACKAGES += {% for pkglist in pkglists %}$(INCLUDE_{{ pkglist.name }}){% if not loop.last %} {% endif %}{% endfor %}
{%- endfor %}
endif""").render(
            target=self.name,
...
targets.get('lantiq-xrx200'). \
    ...
    add_pkglist(PKGS_USB_STORAGE). \
    add_pkglist(PKGS_TLS). \
    exclude([ # devices without usb ports or not enough usable firmware space
        'avm-fritz-box-7412',
        'tp-link-td-w8970',
    ], pkglists=[PKGS_USB, PKGS_USB_NET, PKGS_USB_SERIAL, PKGS_USB_STORAGE])
